...
import bitcoin.exception.BitcoinBlockReadException;
import bitcoin.structure.BitcoinUtil;  /* cross dependencies between formats NEED A SOLUTION*/
...
public List<BitcoinTransactionOutput> parseTransactionOutputs(ByteBuffer rawByteBuffer, long noOfTransactionOutputs) {
	...
	for (int i=0;i<noOfTransactionOutputs;i++) {
		...
		BigInteger currentTransactionOutputValue = new BigInteger(1,BitcoinUtil.reverseByteArray(currentTransactionOutputValueArray));
