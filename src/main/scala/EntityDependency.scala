...
object EntityDependency extends Serializable{
  ...
  def run(sc: SparkContext,inputNodes: RDD[(Long, List[String])],inputEdges: RDD[(String,Long)]): RDD[(Long, List[String])] = {
    ...
    val entityGraph  = Graph.fromEdgeTuples(mappedEdges,1).cache
    
    val connectedEntity =  entityGraph.connectedComponents().vertices
    .aggregateByKey(ListBuffer.empty[Long])(
        (numList, num) => {numList += num; numList},
         (numList1, numList2) => {numList1.appendAll(numList2); numList1})

    
    val broadNodes = sc.broadcast(inputNodes.collectAsMap)    
    val result = connectedEntity.map(id => (id._1,(broadNodes.value.get(id._1).getOrElse(List[String]()) ++ id._2.flatMap(broadNodes.value.get(_).getOrElse(List[String]())))))
                    .mapValues(_.distinct)
    result;
  } 
 
  def betterRun(sc: SparkContext,inputNodes: RDD[(Long, List[String])],inputEdges: RDD[(String,Long)],output:String): RDD[(Long, List[String])] = {
    ...
    //return id -> cluster
    val cc = ConnectedComponent.alternatingAlgo(mappedEdges, 9999999, 9999999, false, 0, 20)._1
    
    val reversecc = cc.map(f => (f._2,f._1))
